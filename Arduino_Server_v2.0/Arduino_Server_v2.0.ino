...
void loop() {
  ...
  if (client) {
    ...
    if (index > 0 && _buffer[index - 1] == '\0') {
      Serial.println();
      if (compArray(readTemp.c_str(), readTemp.length(), _buffer) == true) {
        // Sends temperature to client
        DHT.read11(sensor);
        int temp = (int)DHT.temperature;
        server.println(temp);
      } else if (compArray(readHum.c_str(), readHum.length(), _buffer) == true) {
        // Sends humidity to client        
        DHT.read11(sensor);
        int hum = (int)DHT.humidity;
        server.println(hum);
      } else if (compArray(_update.c_str(), _update.length(), _buffer) == true) {
        DHT.read11(sensor);
        int temp = (int)DHT.temperature;
        int hum = (int)DHT.humidity;
        ...
        Serial.println(s);
        server.print(s); 
      } else if (compArray(discon.c_str(), discon.length(), _buffer) == true) {
